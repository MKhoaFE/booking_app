...
const Booking = () => {
  ...
  const journeyId = storedTravelData?.journeyId;

  const userCookie = Cookies.get("user");
  const userId = userCookie
    ? JSON.parse(decodeURIComponent(userCookie))?.id
    : null;
    ...
    const fetchSeats = async () => {
      try {
        const reservedRes = await axios.get(
          `http://localhost:5000/api/trainSchedule/${journeyId}`
        );
        const lockedRes = await axios.get(
          `http://localhost:5000/get/getLockedSeats/${journeyId}`
        );
        setReservedSeats(reservedRes.data.reservedSeats || []);
        setLockedSeats(lockedRes.data.lockedSeats || []);
      } catch (error) {
    ...
    const fetchLockedSeats = async () => {
      try {
        const res = await axios.get(
          `http://localhost:5000/api/getLockedSeats/${journeyId}`
        );
        setLockedSeats(res.data.lockedSeats || []);
      } catch (err) {
        console.error("Lỗi load locked seats:", err);
      }
    };

    fetchLockedSeats();
    const interval = setInterval(fetchLockedSeats, 5000);
  ...
  const fetchJourneyData = async (journeyId) => {
    try {
      ...
      const storedSeats =
        JSON.parse(
          localStorage.getItem(`selectedSeats_${journey.departureTime}`)
        ) || {};
      const specialSelected = Object.values(storedSeats).filter(
        (type) => type === "special"
      ).length;
      const regularSelected = Object.values(storedSeats).filter(
        (type) => type === "regular"
      ).length;
      ...
      const newPriceInfor = {
        [journey.departureTime]: {
          special: {
            ...
            remaining:
              journey.specialSeats -
              (journey.specialTicketBooked || 0) -
              specialSelected,
          ...
          regular: {
            ...
            remaining:
              journey.regularSeats -
              (journey.regularTicketBooked || 0) -
              regularSelected,
  ...
  //select seat (lock seat)
  const handleSeatSelection = async (seat, ticketType) => {
    try {
      const res = await axios.post("http://localhost:5000/api/lock-seat", {
        journeyId: journeyId, // bạn cần có biến journeyId ở component này
        userId: userId,
        seat: seat,
      });
  ...
  //hủy select ghế (release ghế)
  const unselectSeat = async (seat) => {
    const ticketType = selectedSeats[seat];
    if (!ticketType) return;

    try {
      await axios.post("http://localhost:5000/api/release-lock", {
        journeyId,
        seat,
      });
    } catch (err) {
                  ...
                  <div className="tab-content roboto-regular">
                    <div className="prices">
                      ...
                      <div className="item">
                        ...
                        <div className="price">
                          {currentPriceInfor.special?.regularTicketPrice?.toLocaleString() ||
                            0}{" "}
                          VND
                        </div>
                      </div>
                      <div className="item">
                        ...
                        <div className="price">
                          {currentPriceInfor.special?.specialTicketPrice?.toLocaleString() ||
                            0}{" "}
                          VND
                        </div>
                      </div>
                    ...
                    <div className="prices">
                      ...
                      <div className="item">
                        <div className="name">Giá vé (người lớn):</div>
                        <div className="price">
                          {currentPriceInfor.regular?.regularTicketPrice?.toLocaleString() ||
                            0}{" "}
                          VND
                        </div>
                      </div>
                      <div className="item">
                        <div className="name">Giá vé (trẻ em):</div>
                        <div className="price">
                          {currentPriceInfor.regular?.specialTicketPrice?.toLocaleString() ||
                            0}{" "}
                          VND
                        </div>
                      </div>
